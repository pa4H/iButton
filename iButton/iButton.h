...
#define cHigh 84
  void emulateCyfral(byte *_cyfKey)
  {
    for (byte i = 0; i < 5; i++) // У Cyfral код состоит из 4 байт
    {
      ...
      {
        if (i == 4 && b == 4)
        { // В последнем байте у Cyfral только 4 левых бита
          break;
        }
        MOW_CLI();
  ...
  bool waitForPresence()
  {
    ...
    while (!digitalRead(iButtonPin) && millis() - timeout < 300)
      ; // Ждём пока идёт reset
    MOW_CLI();
    ...
    {
      return false;
    } // Если линия прижата к земле, значит идёт reset
    return true;
  }
  ...
  byte readSlave()
  {
    ...
    for (byte i = 8; i; i--)
    {
      ...
      timeout = millis();
      while (!digitalRead(iButtonPin) && millis() - timeout < 300)
        ; // Ничего не делаем пока 0
      timeout = millis();
      while (digitalRead(iButtonPin) && millis() - timeout < 300)
        ;                    // Ничего не делаем пока 1
